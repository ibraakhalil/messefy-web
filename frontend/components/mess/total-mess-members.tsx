...
function getMemberName(member: Member) {
  return member.user?.name?.trim() || member.name?.trim() || 'Unnamed member';
}
...
function MemberListSkeleton() {
  return (
    <div className="tablet:grid-cols-2 grid gap-3" role="status" aria-label="Loading members">
      ...
      <span className="sr-only">Loading all mess members…</span>
    </div>
  );
}
  ...
  const filteredMembers = useMemo(() => {
    return [...members]
      .sort((firstMember, secondMember) => {
        ...
        return (
          roleDifference || getMemberName(firstMember).localeCompare(getMemberName(secondMember))
        );
      })
      .filter((member) => {
        const matchesRole = roleFilter === 'all' || member.role === roleFilter;
        const searchableText = `${getMemberName(member)} ${
          member.isOffline ? '' : member.user?.email || ''
        }`.toLowerCase();

        return matchesRole && searchableText.includes(deferredQuery);
      });
  }, [currentMember?.id, deferredQuery, members, roleFilter]);
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-pure-color text-lg font-semibold">All Mess Members</h2>
                {!isLoading ? (
              ...
              <p className="text-subtitle-color mt-1 text-sm">
                Everyone who belongs to this mess, all in one place.
              </p>
      ...
      <div className="tablet:p-6 p-4">
        {isLoading ? (
          ...
          <div className="border-border-color bg-secondary-bg rounded-xl border p-6 text-center">
            <p className="text-pure-color text-sm font-semibold">Members could not be loaded.</p>
            <button
              type="button"
              onClick={() => refetch()}
          ...
          <div className="border-border-color bg-secondary-bg rounded-xl border px-5 py-10 text-center">
            ...
            <p className="text-pure-color mt-3 text-sm font-semibold">No matching members</p>
            <p className="text-subtitle-color mt-1 text-xs">
          ...
          <>
            <p className="text-subtitle-secondary mb-3 text-xs" aria-live="polite">
                  ...
                  >
                    <div className="relative flex size-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-100 to-teal-100 text-sm font-bold text-emerald-800 dark:from-emerald-900/70 dark:to-teal-900/70 dark:text-emerald-100">
                      ...
                      <span
                        ...
                        title={member.isOffline ? 'Offline member' : 'Registered member'}
                      />
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-subtitle-secondary mt-0.5 truncate text-xs">
                        {member.isOffline
                          ? 'Offline member · no account needed'
                          : member.user?.email || 'Registered member'}
                      </p>
                    </div>
