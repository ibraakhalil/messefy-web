...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', isLoading = false, children, disabled, ...props }, ref) => {
    const variants = {
      ...
      ghost:
        'bg-transparent text-pure-color hover:bg-secondary-bg active:bg-gray-200 dark:text-gray-200 dark:hover:bg-gray-800',
      destructive:
    ...
    return (
      <button
        ref={ref}
        className={cn(
          'flex h-10 cursor-pointer touch-manipulation items-center justify-center gap-2 rounded-lg px-3 text-[15px] transition-[background-color,border-color,color,box-shadow,transform] duration-200 hover:shadow-sm focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50',
          variants[variant],
          className,
        )}
